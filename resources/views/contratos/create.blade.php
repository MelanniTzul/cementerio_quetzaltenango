<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Crear Contrato
        </h2>
    </x-slot>

    <div class="py-8 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-md rounded p-6">
            <form action="{{ route('contratos.store') }}" method="POST">
                @csrf



                <x-input-label for="id_responsable" value="Responsable" />
                <select name="id_responsable" class="form-control mb-4">
                    @foreach($responsables as $r)
                        <option value="{{ $r->id }}">{{ $r->nombre . ' ' . $r->apellido  }}</option>
                    @endforeach
                </select>

                <x-input-label for="id_nicho" value="Nicho" />
                <select name="id_nicho" class="form-control mb-4">
                    @foreach($nichos as $n)
                        <option value="{{ $n->id }}">{{ $n->codigo }}</option>
                    @endforeach
                </select>

                <x-input-label for="id_ocupante" value="Ocupante" />
                <select name="id_ocupante" class="form-control mb-4">
                    @foreach($ocupantes as $o)
                        <option value="{{ $o->id }}">{{ $o->nombre }}</option>
                    @endforeach
                </select>

                <x-input-label for="fecha_inicio" value="Fecha de Inicio" />
                <x-text-input type="date" name="fecha_inicio" class="form-control mb-4" />

                <x-input-label for="fecha_fin" value="Fecha de Fin" />
                <x-text-input type="date" name="fecha_fin" class="form-control mb-4" />

                <x-input-label for="monto" value="Monto" />
                <x-text-input type="number" name="monto" step="0.01" class="form-control mb-4" />

                <x-primary-button>Guardar Contrato</x-primary-button>
            </form>
        </div>
    </div>
</x-app-layout>
